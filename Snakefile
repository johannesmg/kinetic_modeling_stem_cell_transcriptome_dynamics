...
CEL_DIR='path/to/cel_files'
PLDIRECTORY='path/to/profile_likelihood_data'

BOUNDARYFILE='input_data/parameter_bounds_k_strict.csv'
FIT_DATA_FILE='output_data/fit_data_in_limma.csv'
PLOT_DATA_FILE='output_data/fit_data_in_limma.csvnot_sem_corrected.csv'
CHANGING_GENES_FILE='output_data/all_changing_genes_limma.csv'
...
rule all:
 input:
   [expand('figures/panel_01_{n_top_regulated}.pdf',n_top_regulated=N_TOP_REGULATED),expand('figures/panel_02_{n_top_regulated}.pdf',n_top_regulated=N_TOP_REGULATED),expand('figures/panel_03_{n_top_regulated}.pdf',n_top_regulated=N_TOP_REGULATED),expand('figures/supplement_pl_peaktime_parameters_ci_{n_top_regulated}.pdf',n_top_regulated=N_TOP_REGULATED),expand('figures/panel_04_{sig_thresh_gsea_kegg}_{minsetsize_msigdb}_{minsetsize_kegg}_{n_top_regulated}.pdf',n_top_regulated=1500,minsetsize_kegg=MINSETSIZE_KEGG,minsetsize_msigdb=MINSETSIZE_MSIGDB,sig_thresh_gsea_kegg=SIG_THRESH_GSEA_KEGG)]
...
rule fit_input_data:
  input:
    [ESET_FILE]
  output:
    [FIT_DATA_FILE,CHANGING_GENES_FILE,'output_data/vpa_acute_study_window_treatment_data.csv','output_data/vpa_window_treatment_data.csv',PLOT_DATA_FILE]
  shell:
    'Rscript eset_to_fit_input_limma_error.R {ESET_FILE} {output[0]}'
...
rule create_gsea_model_data_input:
  input:
    [PARAMETER_FILE,FIT_DATA_FILE,CHANGING_GENES_FILE]
  output:
...
rule create_window_data:
  input:
    [CHANGING_GENES_FILE,'output_data/vpa_acute_study_window_treatment_data.csv','output_data/vpa_window_treatment_data.csv']
  output:
...
rule panel1:
  input:
    [PLOT_DATA_FILE,CHANGING_GENES_FILE,SYMBOL_CONVERSION_FILE]
  output:
    'figures/panel_01_{n_top_regulated}.pdf'
  shell:
    'Rscript cowplot_introduction_panel.R {input[0]} {input[1]} {input[2]} {wildcards.n_top_regulated} {LOESS_SPAN} {output}'

rule panel2:
  input:
    [PLOT_DATA_FILE,CHANGING_GENES_FILE,SYMBOL_CONVERSION_FILE,PARAMETER_FILE]
  output:
...
rule panel3:
  input:
    [PLOT_DATA_FILE,CHANGING_GENES_FILE,SYMBOL_CONVERSION_FILE,PARAMETER_FILE,RAW_DATA_PCR]
  output:
...
rule pl_supplement:
  input:
    [PLOT_DATA_FILE,CHANGING_GENES_FILE,SYMBOL_CONVERSION_FILE,BOUNDARYFILE]
  output:
    'figures/supplement_pl_peaktime_parameters_ci_{n_top_regulated}.pdf'
  resources: 
    mem_per_cpu=8000
  threads: 8
  shell:
    'Rscript supplementary_figures_pl.R {input[0]} {input[1]} {input[2]} {wildcards.n_top_regulated} {PLDIRECTORY} {threads} {LOESS_SPAN} {BOUNDARYFILE}'
